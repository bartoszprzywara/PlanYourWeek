...
namespace Remonty
{
    public sealed partial class YourWeek : Page
    {
        ...
        int StartHour = TimeSpan.Parse(LocalDatabaseHelper.ReadItem<Settings>(1).Value).Hours;
        int EndHour = TimeSpan.Parse(LocalDatabaseHelper.ReadItem<Settings>(4).Value).Hours;
        ...
        private void FillPlannedDay1()
        {
            ...
            // znajdź w localdb zaplanowane, ale niezrobione aktywności, które zaczynają się dzisiaj i mają określoną godzinę
            ...
            // znajdź w localdb zaplanowane, ale niezrobione aktywności, które zaczynają się dzisiaj, ale NIE MAJĄ określonej godziny
            ...
            // --------------- KROK 4 ---------------
            // TODO: w kroku nr 4 trzeba dodać zaplanowane, ale niewykonane aktywności z poprzednich dni

            // znajdź w localdb zaplanowane, ale niezrobione aktywności, które zaczynają się przed dzisiaj
            // to czy posiadają określoną godzinę, czy nie - jest obojętne
            // posortuj znalezione aktywności od tych z najwyższym priorytetem
            using (var conn = new SQLite.Net.SQLiteConnection(new SQLite.Net.Platform.WinRT.SQLitePlatformWinRT(), LocalDatabaseHelper.sqlpath))
                tempActivityList = new ObservableCollection<Activity>(conn.Query<Activity>(
                    "SELECT * FROM Activity " +
            ...
            foreach (var act in tempActivityList)
            {
                ...
                // jeśli aktywność zostanie zaproponowana później, niż koniec dnia - zaproponuj ją mimo wszystko
                    ...
                    PlannedDay1.Add(new PlannedActivity(PlannedDay1[i].Id + 1, LocalDatabaseHelper.ReadItem<Activity>(act.Id), "Red"));
                // aktywność, która zostanie wtedy zaproponowana po północy - traktuj mimo wszystko jako dzisiejszą
                else
                    PlannedDay1.Add(new PlannedActivity(PlannedDay1[i].Id + 1 - 24, LocalDatabaseHelper.ReadItem<Activity>(act.Id), "Red"));
            }
        }
